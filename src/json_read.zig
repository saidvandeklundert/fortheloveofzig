...
pub fn main() !void {
    ...
    const human = try read_human_from_json_file(allocator, "data/human.json");
    std.debug.print("Human.name: {s}\nHuman.age: {d}\n", .{ human.name, human.age });
}

// Read target JSON file and return a struct of the type Human:
fn read_human_from_json_file(allocator: Allocator, path: []const u8) !Human {
...
const Human = struct { name: []const u8, age: u8 };
